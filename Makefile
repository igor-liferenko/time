...
c:
	tie -c time.ch time.w txin.ch notification.ch intensity.ch long-display.ch >/dev/null
	ctangle time time
	@make --no-print-directory time

k:
	tie -c time.ch time.w txin.ch notification.ch intensity.ch reverse.ch >/dev/null
	ctangle time time
	@make --no-print-directory time

u:
	tie -c time.ch time.w txin.ch notification.ch intensity.ch >/dev/null
	ctangle time time
	@make --no-print-directory time
